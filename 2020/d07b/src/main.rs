...
fn get_child_count(children: &Nesting, name: &str) -> i64
{
	let mut count = 1;
	for (k, v) in children.get(name).unwrap()
	{
		count += v * get_child_count(children, k);
	}
	count
}
...
fn main()
{
	...
	let count = get_child_count(&children, "shiny gold");
